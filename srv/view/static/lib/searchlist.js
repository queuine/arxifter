...
class SearchList extends React.Component {
  constructor(props) {
    ...
    this.downloadSearch = async (id, rank) => {
      ...
      try {
        const downloadBlob = new Blob([JSON.stringify({
          ...
          "answer": downloadItem?.answers?.flat().map(item => this.sortArticleData(item))
        }, null, 4)], {
    ...
    this.saveLastSearches = (toSave, searchList) => {
      if (toSave ?? this.getToSaveLastSearches()) {
        storageSaveSifts(props.getStoragePrefix(), searchList ?? this.state.searchList, getFabricUi()["recallSifts"]);
      } else {
        storageCleanSifts(props.getStoragePrefix());
      }
    };
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      doSave: () => this.downloadSearch(x.id, searchCount - i),
