...
function ArxifterPage() {
  ...
  const getFeedType = () => {
    const feedType = storageLoadSetupFeedType(getStoragePrefix());
    const knownFeedTypes = [getFabricQuery()["feedTypeCounts"], getFabricQuery()["feedTypeDays"]];
  ...
  const setFeedType = feedType => {
    storageSaveSetupFeedType(getStoragePrefix(), feedType);
  };
  ...
  const setupGuestSession = hasAgreed => {
    ...
    const postUrl = [getFabricView()["pathPrefix"], utilsGetSessionParts().join("/")].join("/");
    fetch(postUrl, {
      method: 'post',
      body: postData,
      headers: {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    ref: searchesRef,
    searchList: storageLoadSifts(getStoragePrefix(), jsonrepair),
    getSaveLastSearches: getSaveLastSearches,
